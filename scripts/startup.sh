...
echo $(whoami) | tee -a output.log
echo "this is the url $${GIT_URL}" | tee -a output.log

GIT_URL="https://github.com/franknaw/azure-runner-poc"
...
echo "--url $GIT_URL" | tee -a output.log
echo "--token $RUNNER_TOKEN" | tee -a output.log
echo "--name $RUNNER_NAME" | tee -a output.log
echo "--work $RUNNER_WORK_DIRECTORY" | tee -a output.log
echo "--labels $RUNNER_LABELS" | tee -a output.log
echo "--runnergroup "$${_RUNNER_GROUP}"" | tee -a output.log

#cat ./svc.sh | tee -a output.log

echo "installed" | tee -a output.log
sudo ./run.sh

sudo ./svc.sh start

echo "configured" | tee -a output.log
sudo ./svc.sh install

echo "started" | tee -a output.log